...
Keep in mind, Node.js is strictly a tool to run JavaScript on a server – while it's possible to build web applications and APIs in straight JS, we'll actually be using a framework on top of Node called Express. It's actually quite similar to Rails - you'll like it!
...
While it means you'll have to think & write your code a little differently than you did with a blocking framework like Rails, the benefit of speed is one thing a lot of folks are excited about with the introduction of Node.
...
console.log("Name is: ", my-module.name)

// Closures are still closures
console.log("The number is: " + my-module.getNumber())

// JavaScript is still JavaScript
console.log("The array contains " + my-module.arr.length + " elements")
...
Though, it's frequently referred to as a package manager, technically, the founders of Node are quite frank when they say: "[npm] is a recursive bacronymic abbreviation for 'npm is not an acronym'." If it were then called NINAA, it _would_ actually be an acronym, and this incorrectly named. Read more [here](https://docs.npmjs.com/misc/faq#if-npm-is-an-acronym-why-is-it-never-capitalized).
