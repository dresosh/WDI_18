...
* Tables
	...
	* Primary Keys
	* Foreign Keys
* Entity relationship diagrams with crow's foot notation for ideas for web apps
* Explain the different relationship types – 
	* has_one (one to one) 
	* has_many / belongs_to (one to many / many to many) 
	* has_many_through
...
>TASK (everyone, 2 minutes): On the walls of the classroom list every database you know from memory. Also, if you can, make a bullet list next to the database name with everything that you already know about this DB. 
...
### Ralational v Document based v Others

>TASK (everyone, 5 minutes): Spend 2 minutes googling on databases and add to the list on the classroom walls. Focus on trying to understand what a "relational DB" might be, compared to a "document based" 

In modern web development, there are different categories of databases – SQL, NoSQL, Key/Value, graph. We're focusing on SQL because historically that's the father to the others, but we will talk about the others later in the course.
...
TASK > 10 miutes > Let's say we're making an app for a library and look at what some tables would look like (e.g. what information or attributes would be associated with each table?)
